...
export default function ReadmeGenerator() {
  ...
  const generateReadme = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post('http://localhost:3000/user/readme', {
        url: githubUrl
      }, {
        headers: {
      ...
      const content = response.data.readme.replace(/```markdown\n([\s\S]*?)```/, '$1');
  ...
  return (
    <div className="min-h-screen bg-gray-900">
        ...
        <meta name="description" content="Generate beautiful README files from GitHub URLs" />
      ...
      <main className="container mx-auto px-4 py-12">
        <div className="text-center mb-12">
          ...
          <p className="text-gray-400 max-w-2xl mx-auto">
            Generate professional README files automatically by just providing your GitHub repository URL
          </p>
        </div>
          ...
          {!readmeContent && !isLoading && (
            <div className="border-2 border-dashed border-gray-700 rounded-xl p-12 text-center">
              <div className="max-w-md mx-auto">
                ...
                <h3 className="mt-2 text-lg font-medium text-gray-400">No README generated yet</h3>
                <p className="mt-1 text-sm text-gray-500">
                  Enter a GitHub repository URL above to generate a beautiful README
                </p>
              </div>
            </div>
          )}
          ...
          {isLoading && (
            <div className="flex justify-center items-center h-64">
              <div className="animate-pulse flex flex-col items-center">
                <svg className="h-12 w-12 text-purple-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                  <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                  <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                </svg>
                <span className="mt-4 text-gray-400">Analyzing repository and generating README...</span>
              </div>
            </div>
          )}
        </div>
